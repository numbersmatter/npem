...
import { MenuIcon, BellIcon, XSquare } from 'lucide-react'
...
export function loader() {
  ...
  const nonprofitDetails = {
    name: 'Food Pantry',
    description: 'Helping those in need with food and resources.',
    logoUrl: '',
  }

  return { user, navigation, userNavigation, nonprofitDetails }

}


export default function StackedLayout({ loaderData }: Route.ComponentProps) {
  const { user, navigation, userNavigation, nonprofitDetails } = loaderData;

  const nonprofitLogo = planthydrating;
            ...
            <div className="flex h-16 justify-between">
              ...
              <div className="-mr-2 flex items-center sm:hidden">
                ...
                <DisclosureButton className="group relative inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:outline-hidden">
                  ...
                  <XSquare aria-hidden="true" className="hidden size-6 group-data-open:block" />
                </DisclosureButton>
              </div>
            </div>
          </div>
